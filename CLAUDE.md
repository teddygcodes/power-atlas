...
## Scope (current state)
- **Built (v0.6):** power resolver, water resolver, cooling→water cascade, flood site-risk, construction-timeline reveal, 3D campus massing, explainability drawer (surface-existing-data); lazy-loaded layers + repCoord geometry decoupling; 118 tests.
...
- **3D campus (`lib/campus/assets.ts` + `components/campus/Campus3D.tsx`) is cosmetic, not analysis.** A react-three-fiber inset reveals schematic massing per build phase, REUSING `PHASE_ORDINAL`/`buildPhase` (timeline logic untouched — `assetsForPhase` mirrors the cumulative pattern, doesn't rewrite it). No resolver calls, no data. Lazy-loaded via `dynamic(ssr:false)` so `three` stays out of the initial bundle.
...
- **`lib/geo/*`, `lib/power/*`, `lib/water/*`, `lib/cooling/*`, `lib/flood/*`, `lib/timeline/*`, `lib/campus/*`, `lib/explain/*` are pure/isomorphic** (no node-only imports) — shared by the ingest script and the browser. Keep them that way.
- **Explainability (`lib/explain/explain.ts` + `components/explain/ExplainDrawer.tsx`) SURFACES existing resolver output — it never recomputes.** `explainPower/Water/Flood` read fields already on the output object; reason-code labels are faithful restatements; rawTags are looked up verbatim by `featureId` (`findFeatureRawTags`). Do NOT re-call `voltageClass`/`waterDemandClass`/`floodSeverity` or invent a number/score/rationale here — surface the class via the already-emitted reason code. Flood has no featureId → no rawTags surfaced.
...
- **3D = schematic massing, never a model.** Primitive shapes in ARBITRARY scene units (not meters); plain asset-type labels ONLY (no kV/MWh/gallons/dimensions/per-asset specs). The fake-precision trap is implied design knowledge. Keep the "representative massing — not an architectural or engineering site design" caveat. Stick to Box/Cylinder/Plane (no `THREE.CapsuleGeometry`); no OrbitControls; `frameloop="demand"` + shared materials so 3D never taxes the map.
- **Calibration: hedge genuine unknowns, but DON'T over-hedge true domain facts.** The posture is humility about what the tool can't know (capacity, interconnection feasibility, grid connectivity, water rights, current flood authority) — NOT vagueness about what the domain genuinely knows. A 500MW hyperscale load needing major transmission-level planning is a near-certainty; say "likely/typically required," not "may." Softening true statements to "may" is **false vagueness — its own kind of dishonesty** that erodes credibility with the engineer audience. Likewise keep factual federal/standard terms verbatim (e.g. the FEMA SFHA "1%-annual-chance" definition) — that's a real designation, not a fabricated per-site probability. Confident where the domain is confident; humble where genuinely uncertain.
...
- Coverage scoped to `lib/geo|power|water|cooling|flood|timeline|campus|explain`; CI reports it with **no threshold gate**.
- Flood tests assert **no fabricated magnitude** in output (regex-guard %/gpd/MGD/ft/annual-chance) and that the cached/MSC caveat is always present. Timeline + campus tests similarly regex-guard their output: timeline against any duration/date/schedule value, campus against any per-asset spec (kV/MWh/gallons/dimensions) in labels. WebGL render itself isn't unit-tested — cover the pure asset/phase logic + labels; verify the render in-browser (and trust the DOM, not WebGL screenshots — they capture stale frames).
